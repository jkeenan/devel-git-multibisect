...
=item * Arguments
    ...
    $self->multisect_builds({ probe => 'stderr' });

Optionally takes one hash reference.  At present that hashref may contain only
one element whose key is C<probe> and whose possible values are C<error>,
C<warning>, or C<stderr>.  Defaults to C<error>.  Select among these values
depending on whether you are probing for changes in errors generated by the
C-compiler, changes in warnings generated by the C-compiler, or all text
output to C<STDERR> during F<make>.
