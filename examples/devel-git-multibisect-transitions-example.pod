...
L<https://rt.perl.org/Ticket/Display.html?id=129953>.  Prior to that time
...
i   configure_command:      perl Makefile.PL
    make_command:           make
...
C<harness> for $test_command is actually an alias to:
...
C<process_options()> returns hash ref which we pass to the
...
The output of the target was unchanged from index 0 to index 54, from index 55
to index 58, from index 59 to index 64, and from index 65 to index 219.
